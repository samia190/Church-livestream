...
import { verifyPaypalWebhook } from "./payments";

function callbackValue(items: Array<{ Name?: string; Value?: string | number }> | undefined, name: string) {
...
export function registerPaymentRoutes(app: Express) {
  app.post("/api/payments/mpesa/callback", async (req, res) => {
    try {
      ...
      const donation = await findDonationByProviderReference(checkoutRequestId) as any;
      if (!donation) return res.status(200).json({ ResultCode: 0, ResultDesc: "Accepted" });
      if (callback.ResultCode !== 0) {
        await updateDonationStatus(donation._id.toString(), "failed");
        return res.status(200).json({ ResultCode: 0, ResultDesc: "Accepted" });
      }
      const metadata = callback.CallbackMetadata?.Item as Array<{ Name?: string; Value?: string | number }> | undefined;
      ...
      if (!receipt || amount !== Number(donation.amount)) {
        await updateDonationStatus(donation._id.toString(), "failed");
        return res.status(400).json({ ResultCode: 1, ResultDesc: "Payment details did not match donation" });
      }
      await updateDonationStatus(donation._id.toString(), "completed", String(receipt));
      return res.status(200).json({ ResultCode: 0, ResultDesc: "Accepted" });
    } catch (error) {
  ...
  app.post("/api/payments/paypal/webhook", async (req, res) => {
    try {
      ...
      if (event.event_type === "PAYMENT.CAPTURE.COMPLETED") {
        ...
        const donation = orderId ? await findDonationByProviderReference(orderId) as any : undefined;
        if (donation && event.resource?.id) await updateDonationStatus(donation._id.toString(), "completed", event.resource.id);
      }
