...
interface MediaItem {
  ...
  url: string; // blob: URL for files, or remote URL
  label: string;
...
export default function PreStreamMediaPlayer({ onMediaActivate, isLive }: PreStreamMediaPlayerProps) {
  ...
  const startPlayback = useCallback(() => {
    ...
    if (current.type === "image") {
      // Show image for 10 seconds then auto-advance
      ...
      // For video, we don't switch the preview away from camera — 
      // the user can see the video in the player below
