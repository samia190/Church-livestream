...
import {
  ...
  Image, Music, Film, Volume2, VolumeX, Maximize2, X, Repeat, Shuffle,
  AlertCircle
} from "lucide-react";
...
export interface PreStreamMediaPlayerRef {
   ...
   * Returns null if no media is playing or capture fails.
   */
  ...
  ({ onMediaActivate, isLive }, ref) => {
    ...
    const captureMediaStream = async (): Promise<MediaStream | null> => {
      ...
      try {
        ...
        // Create MediaStream from captured tracks
     ...
     */
    const captureVideoTrack = async (): Promise<MediaStreamVideoTrack | null> => {
      ...
      return videoTrack as MediaStreamVideoTrack;
    };

    /**
     * Capture audio from audio element using Web Audio API.
     */
    const captureAudioTrack = async (audioElement: HTMLAudioElement): Promise<MediaStreamAudioTrack | null> => {
      try {
        ...
        return audioTrack as MediaStreamAudioTrack;
      } catch (error) {
        console.error("[PreStreamMediaPlayer] Audio capture error:", error);
        // Return null instead of throwing to allow video-only capture
        return null;
      }
    };
    ...
    const stopCapture = useCallback(() => {
      ...
      // Stop all tracks in capture stream
