...
import {
  ...
  Monitor, Wifi, WifiOff, Film, Play, Pause
} from 'lucide-react';
...
export const ModernLiveStudio: React.FC = () => {
  ...
  const handlePreStreamMediaActivate = (mediaStream: MediaStream | null) => {
    if (mediaStream) {
      // Switch video preview to show the media
      if (videoRef.current) {
      ...
      <Tabs defaultValue="cameras" className="space-y-4">
        <TabsList className="grid w-full grid-cols-5 bg-card/60 border border-border">
          ...
          <TabsTrigger value="media" className="flex items-center gap-1">
            <Film className="w-4 h-4" />
            <span className="hidden sm:inline">Pre-Stream</span>
          </TabsTrigger>
        ...
        <TabsContent value="cameras" className="space-y-4">
          <Card className="p-4 glass-panel border-0">
              ...
              {camera.isScanning && (
                ...
                >
                  <div className="p-4 rounded-lg bg-blue-500/10 border border-blue-500/30">
                    ...
                    <div className="flex justify-between mt-1 text-[10px] text-blue-400/70">
                      <span>{camera.scanProgress < 30 ? "Initializing device scan..." :
                           camera.scanProgress < 50 ? "Detecting USB devices..." :
                           camera.scanProgress < 70 ? "Checking HDMI capture cards..." :
                           camera.scanProgress < 90 ? "Enumerating video inputs..." :
                           camera.scanProgress < 100 ? "Resolving device labels..." :
                           "Scan complete"}</span>
                      <span>{camera.scanProgress}%</span>
                    </div>
                  </div>
                </motion.div>
              )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4">
              {camera.devices.length === 0 && !camera.isScanning && (
                <div className="col-span-2 p-6 rounded-lg border-2 border-dashed border-border/50 text-center">
                  <Camera className="w-10 h-10 text-muted-foreground/40 mx-auto mb-2" />
                  <p className="text-sm text-muted-foreground mb-1">
                    No cameras detected yet
                  </p>
                  <p className="text-xs text-muted-foreground/60">
                    Grant camera permission or click "Scan Devices" to detect USB webcams and HDMI capture cards
                  </p>
                </div>
              )}
                ...
                >
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${
                        device.isCapturing ? 'bg-red-500 animate-pulse' : 'bg-green-500'
                      }`} />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-muted-foreground">
                      {device.isCapturing ? 'Active stream' : 'Available'}
                    </span>
                  </div>
                </motion.button>
              ))}
            ...
            {camera.devices.length > 0 && (
              <div className="mt-3 p-2 rounded bg-primary/5 border border-primary/10">
                <p className="text-xs text-muted-foreground flex items-center gap-1">
                  ...
                  USB webcams and HDMI capture cards (via USB dongle) appear here automatically
                </p>
              </div>
            )}
          </Card>
          ...
          <Card className="p-4 glass-panel border-0">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button
                ...
                className={showPreStream ? "bg-ember text-ember-foreground" : ""}
              >
                {showPreStream ? (
        ...
        <TabsContent value="platforms" className="space-y-4">
          <Card className="p-4 glass-panel border-0">
            ...
            <p className="text-xs text-muted-foreground mb-4">
              Connections are saved to the database. Streaming to these platforms additionally
              requires a Restream.io account or similar configured on the server.
            </p>
