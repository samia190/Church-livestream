...
export default function FaithJournal() {
  ...
  const createEntry = trpc.journal.create.useMutation({
    onSuccess: () => {
      ...
      setMood(undefined);
      toast.success("Your reflection was saved privately.");
    },
  });
  const removeEntry = trpc.journal.remove.useMutation({
        ...
        <main className="pt-32 px-4">
          <Card className="max-w-xl mx-auto p-8 text-center glass-panel border-0">
            ...
            <h1 className="text-3xl font-bold mb-3">
              Your private faith journal
            </h1>
            <p className="text-muted-foreground mb-6">
              Sign in to keep your prayers, reflections, and Scripture insights
              private and available across your devices.
            </p>
            <Button
              onClick={() =>
      ...
      <main className="pt-28 pb-20 px-4">
        <div className="max-w-5xl mx-auto">
          <motion.div
            ...
            className="mb-10"
          >
            <p className="label-eyebrow mb-3">A quiet place to remember</p>
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              Faith Journal
            </h1>
            <p className="text-lg text-muted-foreground max-w-2xl">
              Write honestly before God. Your entries are private by default and
              belong to you.
            </p>
          ...
          <div className="grid lg:grid-cols-[0.9fr_1.1fr] gap-8 items-start">
            <Card className="p-6 glass-panel border-0 lg:sticky lg:top-24">
              <div className="flex items-center gap-3 mb-5">
                <BookHeart className="w-6 h-6 text-ember" />
                <h2 className="text-xl font-bold">Write a reflection</h2>
              </div>
              <form onSubmit={submit} className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {moods.map(item => (
                    <button
                      ...
                      className={`rounded-full px-3 py-1 text-xs border capitalize ${mood === item ? "border-ember bg-ember/10 text-ember" : "border-border text-muted-foreground"}`}
                    >
                      {item}
                    </button>
                  ))}
                ...
                <Textarea
                  ...
                  placeholder="What is God bringing to your heart today?"
                  rows={8}
                  maxLength={12000}
                  required
                />
                ...
                >
                  ...
                  {createEntry.isPending ? "Saving…" : "Save privately"}
                </Button>
              </form>
            </Card>
            <section>
              {isLoading ? (
                ...
                <Card className="p-8 text-center glass-panel border-0">
                  <BookHeart className="w-10 h-10 text-ember mx-auto mb-4" />
                  <h2 className="text-xl font-bold mb-2">
                    Your journal is waiting
                  </h2>
                  <p className="text-muted-foreground">
                    Your first entry could be a prayer, a question, a gratitude,
                    or a Scripture that stayed with you.
                  </p>
                </Card>
              ) : (
                <div className="space-y-4">
                  {entries.map(entry => (
                    <Card key={String(entry._id)} className="p-6 border-border">
                      <div className="flex justify-between gap-4">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (
                              window.confirm(
                                "Delete this private journal entry?"
                              )
                            )
                              removeEntry.mutate({ id: String(entry._id) });
                          }}
                        >
                      ...
                      {entry.scriptureReference && (
                        <p className="text-sm text-ember mt-4">
                          {entry.scriptureReference}
                        </p>
                      )}
